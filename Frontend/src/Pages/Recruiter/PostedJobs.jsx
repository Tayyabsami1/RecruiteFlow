...
const PostedJobs = () => {
    ...
    const fetchRecruiterId = async () => {
      try {
        const res = await axios.get(
          `/api/recruiter/getRecruiterId/${User._id}`
        ); //Same logic as in PostJob.jsx
  ...
  //The two use Effects are used here becaue keeping them together in a single useEffect can cause race conditions. We need recruitedId to fetchJobs; if both of the api calls are in same useEffect then fetchRecruiterId will be called and meanwhile fetchPostedJobs will be called so fetchPostedJobs can return empty array or outdated data. In above version, fetchPostedJobs will only run when there is a recruiter Id or whenever the recruiter id will be changed.
  ...
  return (
    // Main Box
    <Box
      className={`posted-jobs-container ${isDark ? "dark-mode" : ""}`}
      sx={{
        px: { xs: 2, md: 4 },
        py: 3,
        width: "100%",
    ...
    >
      {/* It is used to give notifications (server response messages) to user (frontend) */}
          ...
          {filteredJobs.length > 0 ? (
              ...
              <Grid item xs={12} sm={6} lg={4} key={job._id}>
                <Card
                  ...
                  sx={{
                    ...
                    border: isDark
                      ? "1px solid rgba(120, 122, 145, 0.2)"
                      : "none",
                    "&:hover": {
                ...
                >
                  <CardContent sx={{ flexGrow: 1, p: 3 }}>
                    ...
                    >
                      <LocationOn
                        sx={{ color: isDark ? "#EEEEEE" : "#141E61", mr: 1 }}
                      />
                      <Typography
                        variant="body1"
                        sx={{
            ...
            <Grid item xs={12}>
              ...
              >
                ...
                >
                  No jobs posted yet. Create your first job listing to get
                  started!
                </Typography>
                <Button
                  variant="contained"
      ...
      >
        <DialogTitle
          sx={{
            fontWeight: "bold",
            borderBottom: isDark
              ? "1px solid rgba(238, 238, 238, 0.1)"
              : "1px solid rgba(15, 4, 76, 0.1)",
            pb: 2,
          }}
        >
